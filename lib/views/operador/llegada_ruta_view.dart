...
class _LlegadaRutaViewState extends State<LlegadaRutaView> {
  ...
  Future<void> _cargarUsuarioYRegistro() async {
    try {
      ...
      if (user != null) {
        ...
        // Cargar registros del operador actual
        await _cargarUltimoRegistroActivo(user.id);
      } else {
  ...
  /// Cargar el último registro de despliegue activo del operador actual
  Future<void> _cargarUltimoRegistroActivo(int operadorId) async {
    try {
      final db = DatabaseService();

      // Obtener todos los registros de despliegue del operador
      ...
      final registrosDelOperador = todosRegistros
          .where((r) =>
      r.operadorId == operadorId &&
          r.estado == 'DESPLIEGUE')
          .toList();

      print('📊 Registros del operador $operadorId: ${registrosDelOperador.length}');

      if (registrosDelOperador.isNotEmpty) {
        // Tomar el último registro activo
        setState(() {
  ...
  Future<void> _registrarLlegada() async {
    ...
    try {
      ...
      final nuevoRegistroLlegada = RegistroDespliegue(
        ...
        operadorId: _registroActivo!.operadorId,
        sincronizado: false,
      );
      ...
      Future.delayed(const Duration(seconds: 2), () {
        if (mounted && _currentUser != null) {
          _cargarUltimoRegistroActivo(_currentUser!.id);
        }
      });
    } catch (e) {
      ...
      body: _registroActivo == null
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Operador: ${_currentUser?.username ?? "Cargando..."}',
...
//   }
//
//   /// Cargar el último registro de despliegue activo (sin llegada)
//   Future<void> _cargarUltimoRegistroActivo() async {
//     try {
//       final db = DatabaseService();
//       final registrosActivos = await db.obtenerRegistrosActivos();
//
//       if (registrosActivos.isNotEmpty) {
//         // Tomar el último registro activo
//         setState(() {
//           _registroActivo = registrosActivos.last;
//         });
//       } else {
//         _mostrarSnack('No hay registro de despliegue activo', error: true);
//       }
...
//       final ahora = DateTime.now();
//
//       /// 📌 CREAR UN NUEVO REGISTRO DE LLEGADA (NO actualizar el de despliegue)
//       final nuevoRegistroLlegada = RegistroDespliegue(
//         destino: _registroActivo!.destino,
//         latitud: location.latitude.toString(),
//         longitud: location.longitude.toString(),
//         descripcionReporte: _registroActivo!.descripcionReporte,
//         estado: "LLEGADA", // Estado de llegada
//         sincronizar: _sincronizarConServidor,
//         observaciones: _observacionesController.text.isEmpty
//             ? _registroActivo!.observaciones
//             : _observacionesController.text,
...
//       final db = DatabaseService();
//
//       /// 💾 GUARDAR COMO UN NUEVO REGISTRO EN LA BASE DE DATOS
//       final nuevoId = await db.insertRegistroDespliegue(nuevoRegistroLlegada);
//       print('✅ Nuevo registro de llegada creado con ID: $nuevoId');
//
//       // Intentar enviar al servidor si está marcado
